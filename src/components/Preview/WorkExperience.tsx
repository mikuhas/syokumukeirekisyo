...
export const WorkExperience: React.FC<WorkExperienceProps> = ({ data, expIndex }) => {
  ...
  return (
    <table className="experience-table" style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '25px' }}>
      <tbody>
        <tr>
          <td colSpan={2} style={{ padding: '10px', borderBottom: '2px solid #333', backgroundColor: '#f0f0f0' }}>
            <span style={{ fontSize: '0.95rem', marginRight: '15px' }}>{periodDisplay || <span style={{ color: 'red' }}>(期間を入力)</span>}</span>
            <span style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{data.company || <span style={{ color: 'red' }}>(会社名を入力)</span>}</span>
          </td>
        </tr>
        <tr>
