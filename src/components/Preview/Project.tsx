...
export const Project: React.FC<ProjectProps> = ({ data, expIndex, projectIndex }) => {
  ...
  return (
    <table id={`project-${expIndex}-${projectIndex}`} className="project-table" style={{ width: '100%', marginBottom: '20px', borderCollapse: 'collapse', border: '1px solid #b8c4d0' }}>
      <tbody>
        <tr>
          <td colSpan={2} style={{ fontWeight: 'bold', fontSize: '1.05rem', padding: '10px 10px', borderBottom: '2px solid #d1d5db', color: '#162333' }}>
        ...
        <tr>
          ...
          <td style={{ width: '82%', padding: '6px 10px', borderBottom: '1px solid #b8c4d0', fontSize: '0.88rem' }}>
            ...
            <div style={{ borderTop: '1px solid #d0d9e4', paddingTop: '8px' }}>
              ...
              <div style={{ fontWeight: 'bold', fontSize: '0.85rem', marginBottom: '6px', color: '#162333' }}>■ 使用技術</div>
                ...
                {data.techStack ? (
                  <div style={{ fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {Object.keys(PREDEFINED_TECH_STACK).map((category) => {
                      ...
                      return (
                        <div key={category}>
                          <div style={{ fontWeight: 'bold', color: '#162333', marginBottom: '2px', borderLeft: '3px solid #b8c4d0', paddingLeft: '8px' }}>{category}</div>
                          <div style={{ color: '#333', paddingLeft: '12px' }}>{!isEmpty ? items.map(tech => tech.name).join(' / ') : '-'}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ borderTop: '1px solid #d0d9e4', paddingTop: '8px', marginTop: '8px' }}>
                  ...
                  <div style={{ fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {[
                      ...
                      <div key={label}>
                        <div style={{ fontWeight: 'bold', color: '#162333', marginBottom: '2px', borderLeft: '3px solid #b8c4d0', paddingLeft: '8px' }}>{label}</div>
                        <div style={{ whiteSpace: 'pre-wrap', color: '#333', paddingLeft: '12px' }}>{value || '-'}</div>
                      </div>
                    ))}
                  </div>
                </div>
